...
document.querySelector('#createdBy').innerHTML += 'Jerald Malacaste Batal'
...
function buildToDo(todo, index) {
  ...
  let toDoCheckbox =  document.createElement('input')  //3
  ...
  if(todo.complete === true) {
    ...
    toDoText.className = 'completeText' //6
  } 


  
  toDoShell.appendChild(toDoText) //7
  toDoShell.appendChild(toDoCheckbox) //7
...
function displayToDos() {
  ...
  const items = buildToDos(toDoItems)//3.
  for (const display of items) {
    toDoContainer.append(display)
  }
  // code here
...
function addToDo() {
  ...
  const items = new ToDo(toDoInput)
  toDoItems.push({
